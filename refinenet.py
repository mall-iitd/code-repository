...
miou_nd=[]
iteration_list=[]
def transforms_images(image_tensor,batch_size):
    ...
    cr_transforms = transforms.Compose([
        ...
        transforms.ColorJitter(brightness=2, contrast=1, saturation=1, hue=0.001),
        transforms.ToTensor()
    ])
    ...
    for i in range(batch_size-1):
        #print("nikhi")
        #single_image=image_array[0][i]
        image_tensor1 = cr_transforms(image_tensor)
        print(image_tensor1.size())
        image_tensor_list = torch.cat([cr_transforms(image_tensor1).unsqueeze_(0)])
        images.append(image_tensor1.numpy())
    '''
    image_tensor1 = torch.cat([cr_transforms(image_tensor).unsqueeze_(0) 
                              for i in range(batch_size)], dim=0)
    #image_array = np.array(images,dtype=np.float32)
    #print((images))
    #print(image_tensor_list.size())
    #imag = torch.from_numpy(np.stack(images))
    #imag= image_tensor_list
    return image_tensor1.to(device=device)
...
class Trainer(object):
    def __init__(self, args):
        ...
        model = RefineNet(num_classes=self.nclass, pretrained=False)
        ...
        if args.resume is not None:
            ...
            if args.cuda:
                ...
                self.model.module.load_state_dict(checkpoint['dannet_state_dict'])
            else:
    ...
    def test_time_training(self,trainer,dataloaders, epoch,loader):
        ...
        for i, (inputs, labels,filepath) in enumerate(loader):
            ...
            #augmented_image_list=[]
            #augmented_image = transforms_images(image)
            #new_image =augmented_image.numpy()
            #print(augmented_image.size())
            #augmented_image_list.append(new_image)
            #augmented_images.append(augmented_image_list)
            #print(augmented_image.size())
            #augmented_input = torch.tensor(augmented_image_list).cuda()
            ...
            if self.args.densencloss ==0:
                ...
                #single_tensor_image  = torch.from_numpy(numpy_input[0]).type(torch.float32)
                per_sample_iterations=7
                batch_size = len(inputs)
                for i in range(len(inputs)):
                    for k in range(per_sample_iterations):
                        numpy_input=inputs.numpy()
                        single_tensor_image  = torch.from_numpy(numpy_input[i]).type(torch.float32)
                        augmented_images= transforms_images(single_tensor_image,batch_size)
                        new_images= augmented_images.cuda()
                        ...
                        #corrects = torch.sum(preds[0].cuda() == labels[0].data.cuda())
                        #iou.evaluateBatch(preds[0].cuda(), labels[0].data.cuda())
                        #print(corrects)
                    #train_celoss += celoss.item()
                    #output = self.model(image)
