...
### Data helpers
################################
def get_test_trans_sample(mean, std, target_size):
    ...
    def test_trans(image, mask=None):
        ...
        mage = TF.resize(image, target_size, interpolation=1)
        # From PIL to Tensor
...
def get_train_trans(mean, std, target_size):
...
def vislbl_test(label, mask_colors):
    label = label.cpu()
