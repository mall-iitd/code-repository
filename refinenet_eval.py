...
class Trainer(object):
    def __init__(self, args):
        ...
        '''
        num_classes = len(CityscapesExt.validClasses)
        model = RefineNet(num_classes, pretrained=False, invariant=None)
        if torch.cuda.is_available():
            model = torch.nn.DataParallel(model).cuda()
            print('Model pushed to {} GPU(s), type {}.'.format(torch.cuda.device_count(), torch.cuda.get_device_name(0)))
        #print(model)
         # Load weights from checkpoint
        #checkpoint = torch.load('/home/anonymous6295/scratch/CIConv_zero_shot/experiments/rloss/pytorch/pytorch-deeplab_v3_plus/ciconv-master/experiments/3_segmentation/cityscapes_rgb.pth.tar')
        checkpoint = torch.load('/home/anonymous6295/scratch/models/refinenet_model.pth')
...
def main():
    ...
    for epoch in range(trainer.args.start_epoch, trainer.args.epochs):
        ...
        if args.test==1:
             miou=trainer.test_time_training_test(trainer,dataloaders,epoch,loader=dataloaders['test_dz_full'])
