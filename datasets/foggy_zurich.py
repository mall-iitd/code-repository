...
class foggyZurichDataset(Cityscapes):
    ...
    def __init__(self, root, split='val', transforms=None):
        ...
        if split == 'val':
            self.imgs_root = os.path.join(root,'RGB_images/')
            self.masks_root = os.path.join(root,'gtFine/')
            self.masks = [mask for mask in list(sorted(os.listdir(self.masks_root))) if 'frame' in mask]
        else:
            self.imgs_root = os.path.join(root,'RGB/GOPR0229/')
