...
class DenseNCLossFunction(Function):
    ...
    def forward(ctx, images, segmentations, sigma_rgb, sigma_xy, ROIs):
        ...
        W1=np.zeros(segmentations.shape, dtype=np.float32)
        bilateralfilter_batch(images, segmentations, AS, ctx.N, ctx.K, ctx.H, ctx.W, sigma_rgb, sigma_xy)
        bilateralfilter_batch(images, segmentations, W1, ctx.N, ctx.K, ctx.H, ctx.W, sigma_rgb, sigma_xy)
        #print(W1.shape())
        densenc_loss -= np.dot(segmentations, AS)
        denominator = np.dot(np.transpose(segmentations), W1)
        #print(denominator)
        # averaged by the number of images
        densenc_loss /= ctx.N
        densenc_loss /= denominator
        dense_first = np.dot(-1*densenc_loss,W1)
        #print(densenc_loss)
        ctx.denominator = denominator
        ctx.dense_first = dense_first
        ctx.AS = np.reshape(AS, (ctx.N, ctx.K, ctx.H, ctx.W))
        ctx.dense_first = np.reshape(dense_first, (ctx.N, ctx.K, ctx.H, ctx.W))
        return Variable(torch.tensor([densenc_loss]), requires_grad=True)
        
    @staticmethod
    def backward(ctx, grad_output):
        grad_segmentation = -2*grad_output*torch.from_numpy(ctx.AS)/ctx.N
        grad_segmentation = grad_segmentation/ctx.denominator
        grad_segmentation += (grad_output*(ctx.dense_first)/np.dot((ctx.denominator),np.transpose(ctx.denominator)))/ctx.N
...
class DenseNCLoss(nn.Module):
    def __init__(self, weight, sigma_rgb, sigma_xy, scale_factor):
        super(DenseNCLoss, self).__init__()
        self.weight = weight
    ...
    def forward(self, images, segmentations, ROIs):
        ...
        return self.weight*DenseNCLossFunction.apply(
